...
class PagesController extends AppController {
	...
	function filtros(){
		if (!empty($this->data)) {
			Debug($this->data);
			if($albergados = $this->requestAction(
							array(
								'controller' => 'albergados',
								'action' => 'buscarAlbergadosPorFiltros',
								'named' => array('nombre_completo' => $this->data["Persona"]["Nombre"], 
														 'edad_minima' => $this->data["Persona"]["Edad_minima"],
														 'edad_maxima' => $this->data["Persona"]["Edad_maxima"],
														 'casa' => $this->data["Persona"]["Casa"])
							)
				)){
				if($persona['Albergado']['id']){
					$Dependiente = $this->requestAction(
